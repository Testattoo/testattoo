...
class WebDriverEvaluator implements Evaluator {
    ...
    List<MetaInfo> metaInfo(String expression) {
        List<Map> infos = getJson("${removeTrailingChars(expression)}.testattoo({method: 'metaInfos'});")
...
//        new Actions(webDriver).moveToElement(element).build().perform()
    ...
    private String execute(String id, String s) {
        ...
        return (function(jQuery) {
            ...
            if (!jQuery().testattoo) { return '__TESTATTOO_MISSING__'; }
            else {
                $element
        ...
        if (v == '__JQUERY_MISSING__') {
            js.executeScript(this.class.getResource('jquery-3.1.1.slim.min.js').text
                + this.class.getResource('testattoo.js').text)
            registeredScripts.each { js.executeScript(it) }
            return execute(id, s)
        }
        if (v == '__TESTATTOO_MISSING__') {
            js.executeScript(this.class.getResource('testattoo.js').text)
            registeredScripts.each { js.executeScript(it) }
            return execute(id, s)
        }
