...
class ByTest {
    ...
    static void before() {
        ...
        when(meta.metaInfo(ArgumentMatchers.any(Component))).thenReturn(new MetaInfo(id: 'id', node: 'node'))
    }
    ...
    void should_generate_expected_expression() {
        ...
        assert by.expression(cmp) == "\$('[id=\"id\"]').find('option')"

        by = By.js('some_js_expression')

        assert by.expression(cmp) == "some_js_expression"
    }
    ...
    void should_find_sub_components() {
        Component cmp = new Component()
        cmp.meta = meta
        when(config.evaluator.metaInfo("\$('[id=\"id\"]').find('sub_expression')"))
            .thenReturn([
